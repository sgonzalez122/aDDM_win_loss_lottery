function expdata = run_win_block(expdata,eyeLink)
if expdata.etSwitch == 1
    % Print current BLOCK/TRIAL to ET screen
	Eyelink('Command', 'record_status_message "WIN BLOCK %d, TRIAL %d"', expdata.curBlock, expdata.trialNum);
    ...
    while fixationTime < expdata.timeFixationScreen
        if Eyelink('NewFloatSampleAvailable') > 0 && ...
            ...
            if x ~= eyeLink.MISSING_DATA && y ~= eyeLink.MISSING_DATA && evt.pa(eyeUsed+1) > 0
                xFix=x;
                yFix=y;
    ...
    expdata.win_StimProb_left(expdata.trialNum) = ball_prob(1);
    expdata.win_StimProb_right(expdata.trialNum) = ball_prob(2);
    ...
    for ii = 1:ball_prob(1)
        my_circle(expdata.windowPtr, expdata.greenCircle, centers(ii,1),...
    ...
    %% SAVEOUT DATA
    if parResponded == 1
        expdata.win_RT(expdata.trialNum) = reactionTime;
        
        % If LEFT Response
        if keyCode(expdata.keyLeft)
            expdata.win_choice(expdata.trialNum) = 1;
        ...
        elseif keyCode(expdata.keyRight)
            expdata.win_choice(expdata.trialNum) = 2;
